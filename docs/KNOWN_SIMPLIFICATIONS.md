...
| DSP-4 | Supported at command level | Complete arithmetic, track/polygon/sprite projection, OAM, lookup, streaming-resume, termination, and mirrored mapper behavior is implemented. Deterministic command tests cover the cartridge interface; Top Gear 3000 has completed pre-race retail bring-up, but active-race validation remains manual. Internal uPD77C25 instruction timing is not modeled. |
...
- DSP-4's automated retail path covers Top Gear 3000's long introduction and
  menu/setup flow, but has not yet entered a race or produced retail DSP-4
  command traffic. An active-race manual smoke test and replayable comparison
  remain desirable.
